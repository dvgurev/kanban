...
На данный момент автоматические тесты отсутствуют. Ручное тестирование можно проводить с помощью инструментов вроде **Postman** или **cURL**, проверяя каждый эндпоинт.

**Пример тестирования эндпоинта `GET /api/board`:**
```bash
curl http://localhost:5000/api/board
```

**Планы по тестированию:** В будущем рекомендуется добавить unit‑тесты для контроллеров и интеграционные тесты для API с использованием **Jest** и **Supertest**.

## 13. Возможные улучшения

1. **Аутентификация и авторизация** – добавление пользователей, разделение досок по пользователям.
2. **Валидация запросов** – использование библиотеки вроде **Joi** или **express‑validator** для строгой валидации входных данных.
3. **Миграции базы данных** – замена `sync({ alter: true })` на систему миграций Sequelize для безопасного обновления схемы в production.
4. **Кэширование** – использование Redis для кэширования часто запрашиваемых данных (например, всей доски).
5. **Пагинация и фильтрация** – если количество задач станет очень большим.
6. **WebSocket** – реализация real‑time обновлений доски для многопользовательского режима.
7. **Документация API** – автоматическая генерация OpenAPI‑спецификации (Swagger).
8. **Контейнеризация** – создание Docker‑образа для упрощения развёртывания.

## 14. Примечания по развёртыванию

### Heroku / Railway / Render
1. Установите переменные окружения в панели управления облачным сервисом.
2. Убедитесь, что база данных PostgreSQL предоставлена (например, Heroku Postgres).
3. Используйте `npm start` в качестве команды запуска.

### Docker
Пример `Dockerfile` для сервера:
```dockerfile
FROM node:18-alpine
WORKDIR /app
COPY package*.json ./
...
Затем можно собрать образ и запустить контейнер вместе с контейнером PostgreSQL.

### Nginx как reverse proxy
В production рекомендуется размещать сервер за reverse proxy (Nginx) для обработки статики, SSL‑терминирования и балансировки нагрузки.

## 15. Лицензия

Проект распространяется под лицензией **MIT**. Подробности см. в файле `LICENSE` в корне репозитория.
